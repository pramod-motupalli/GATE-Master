...
export default function LoginPage() {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      const res = await fetch('https://gate-master-backend.onrender.com/login', {
        method: 'POST',
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-blue-300 px-4">
      ...
      >
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          Don't have an account? <button onClick={() => navigate("/signup")} className="text-blue-600 underline cursor-pointer">
      Sign up
    </button>
        </p> 
        <div className='text-center'>
         <button onClick={() => navigate("/resetpassword")} className="text-blue-500 underline cursor-pointer">
      reset password
    </button>
    </div>
      </form>
