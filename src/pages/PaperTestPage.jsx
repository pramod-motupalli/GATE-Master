...
export default function PaperTestPage() {
    ...
    const handleSubmit = useCallback(
        async (isAutoSubmit = false) => {
                ...
                if (q.type === "msq") {
                    ...
                    isCorrect =
                        ...
                        userAnswerSorted.every(
                            (val, index) => val === correctAnswerSorted[index]
                        );
                } else {
            ...
            try {
                ...
                await axios.post(
                    "https://gate-master-backend.onrender.com/api/attempts",
                    attemptData,
                    {
                        headers: { Authorization: `Bearer ${token}` },
                    }
                ...
                alert(
                    "There was an error saving your results. Please check your connection and try again."
                );
            }
        },
    ...
    useEffect(() => {
        if (isReviewMode) {
            ...
            setResponses(
                Object.fromEntries(
                    new Map(Object.entries(reviewData.responses))
                )
            );
        ...
        setResponses((prev) => {
            if (questions[currentQuestionIndex].type === "msq") {
                ...
                if (checked) {
                    ...
                    return {
                        ...prev,
                        [questionId]: prevResponse.filter((v) => v !== value),
                    };
                }
            } else {
    ...
    // --- SCORE/REVIEW SCREEN ---
    if (showScore) {
        const total = isReviewMode
            ? location.state.reviewData.totalQuestions
            : questions.length;
                ...
                <div className="max-w-4xl mx-auto">
                    <header className="text-center mb-8 p-6 bg-gray-800 rounded-xl shadow-lg">
                        <h1 className="text-3xl font-bold text-cyan-400">
                            {isReviewMode ? "Test Review" : "Test Completed!"}
                        </h1>
                        ...
                        <div className="md:col-span-2 grid grid-cols-2 gap-4 text-center">
                            {[
                                ...
                                },
                                {
                                    label: "Incorrect",
                                    value:
                                        Object.keys(responses).length - score,
                                    color: "text-red-400",
                                },
                                {
                                    label: "Attempted",
                                ...
                                <div
                                    key={item.label}
                                    className="bg-gray-800 p-4 rounded-lg flex flex-col justify-center shadow-lg"
                                >
                    ...
                    <div className="space-y-4">
                        {questions.map((q, i) => {
                            ...
                            let isCorrect = false;
                            if (q.type === "msq") {
                                const userAnswerSorted = Array.isArray(
                                    userAnswer
                                )
                                    ? [...userAnswer].sort()
                                    : [];
                                ...
                                >
                                    ...
                                    {!isCorrect && (
                                        <p className="text-green-400">
                                            ...
                                            {formatAnswerForDisplay(q.answer)}
                                        </p>
                                    )}
                                </div>
                            );
                    ...
                    <div className="text-center mt-8">
                        <button
                            ...
                            className="bg-cyan-600 hover:bg-cyan-700 text-white px-8 py-3 rounded-lg font-semibold text-lg"
                        >
                            {isReviewMode
                                ? "Back to Attempts"
                                : "View All Attempts"}
                    ...
                    <div className="space-y-3">
                        {currentQuestion.type === "mcq" &&
                            currentQuestion.options.map((option, i) => (
                                <label
                                    key={i}
                                    className="flex items-center p-4 bg-gray-900 rounded-lg cursor-pointer hover:bg-gray-700 border-2 border-transparent has-[:checked]:border-cyan-500 transition-all"
                                >
                                    <input
                                        type="radio"
                        ...
                        {currentQuestion.type === "nat" && (
                            <input
                                ...
                                className="border p-3 w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 text-lg"
                ...
                <div className="lg:col-span-1 bg-gray-800 p-6 rounded-xl shadow-2xl flex flex-col">
                    <div className="border-b border-gray-700 pb-4 mb-4">
                        <h3 className="text-lg font-bold text-white text-center">
                            GATE Paper: {subjectId.toUpperCase()}
                        </h3>
                        ...
                        <div className="grid grid-cols-5 sm:grid-cols-6 lg:grid-cols-5 gap-2">
                            {questions.map((_, index) => (
                                <button
                                    ...
                                    className={`flex items-center justify-center h-10 w-10 rounded-md font-bold text-sm transition-all ${getQuestionStatus(
                                        index
                                    )}`}
                                >
                                    {index + 1}
                    ...
                    <div className="mt-6 space-y-3">
                        <div className="flex justify-between">
                            <button
                                ...
                                className="w-full mr-2 bg-gray-600 hover:bg-gray-500 disabled:bg-gray-700 disabled:cursor-not-allowed text-white px-5 py-3 rounded-lg font-semibold transition-colors"
                            >
                                Back
                            </button>
                            <button
                                ...
                                className="w-full ml-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800/50 disabled:cursor-not-allowed text-white px-5 py-3 rounded-lg font-semibold transition-colors"
                            >
                                Next
                            </button>
                        </div>
                        <button
                            ...
                            className="w-full bg-green-700 hover:bg-green-600 text-white px-5 py-3 rounded-lg font-bold text-lg transition-colors"
                        >
                            Submit Test
                        </button>
                    </div>
