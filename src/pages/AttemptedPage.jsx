    ...
    if (!reviewData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
                <h1 className="text-2xl font-bold text-red-500">No Review Data Found</h1>
                <p className="text-gray-400 mt-2">
                    Please access this page by clicking on an attempt from your history.
                </p>
                <Link to="/attempted" className="mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">
                    Go to My Attempts
                </Link>
            </div>
        );
        ...
        <div className="bg-gray-900 text-white min-h-screen p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <header className="mb-8">
                    <button onClick={() => navigate('/attempted')} className="flex items-center gap-2 text-cyan-400 hover:underline mb-4">
                        <FaArrowLeft />
                        Back to My Attempts
                    </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 text-center">
                    <div className="bg-gray-800 p-4 rounded-lg">
                        <p className="text-3xl font-bold text-cyan-400">{score}/{totalQuestions}</p>
                        <p className="text-sm font-semibold text-gray-400">Score</p>
                    </div>
                     <div className="bg-gray-800 p-4 rounded-lg">
                        <p className="text-3xl font-bold text-green-500">{score}</p>
                        <p className="text-sm font-semibold text-gray-400">Correct</p>
                    </div>
                     <div className="bg-gray-800 p-4 rounded-lg">
                        <p className="text-3xl font-bold text-red-500">{incorrectCount}</p>
                        <p className="text-sm font-semibold text-gray-400">Incorrect</p>
                    </div>
                     <div className="bg-gray-800 p-4 rounded-lg">
                        <p className="text-3xl font-bold text-gray-500">{unattemptedCount}</p>
                        <p className="text-sm font-semibold text-gray-400">Unattempted</p>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {questions.map((question, index) => {
                        ...
                        return (
                            <div key={question.id} className={`p-4 rounded-lg border-l-4 ${isCorrect ? 'border-green-500' : 'border-red-500'} bg-gray-800`}>
                                ...
                                <div className="pl-4 border-l-2 border-gray-700 space-y-2">
                                    ...
                                    {!isCorrect && (
                                        <p className="text-green-400">
                                            <span className="font-bold">Correct Answer:</span> {formatAnswerForDisplay(correctAnswer)}
                                        </p>
                                    )}
                                </div>
                            </div>
